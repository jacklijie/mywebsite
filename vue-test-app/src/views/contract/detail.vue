    ...
    <div class="contract-box">
        ...
        <section class="con" id="scrollObj" :class="{'ios-con':isIos,'plus':!isplus}">
            <div class="img-box" :style="frameBoxStyle">
                <iframe class="img-list" srcolling="auto" :src="frameUrl" :style="frameStyle"></iframe>
            </div>
        ...
        <footer>
            <div class="foot-box">
                <div class="foot-table">
                    <span v-for="(item,index) in contractInfoList" :class="{'active':index==curTabIndex}" :key="index" @click="initToken(item.cloudcontractId,index)" v-text="item.contractName"></span>
                </div>
            </div>
        </footer>
...
export default {
    ...
    methods: {
            ...
            ajax.post(link.getSign, {
                ...
                if (res.data && res.data.response && res.data.response.result) {
                    if (res.data.response.result == "0") {
                        _this.isSign = false;
                        modal.valert(_this, res.data.response.reason);
                        this.initToken(this.contractInfoList[this.curTabIndex].cloudcontractId);
         ...
         * 获取token并初始化云合同sdk
         */
        initToken(contractId, index) {
            if (index || index === 0) this.curTabIndex = index;
            let _this = this;
            modal.loading(this, true);
            ajax.post(link.queryToken, {
                contractId: _this.$route.params.contractid
